...
export default function Topup() {
  ...
  const handleBuy = async (pkg: typeof TOPUP_PACKAGES[number]) => {
    ...
    setResult(null);
    try {
      const res = await api.creditTopup(user.id, pkg.id, pkg.dc + pkg.bonus);
      await refresh();
      setResult({
        type: "ok",
        pkg: pkg.id,
      ...
      {user && (
        <div className="mb-6 flex items-center gap-4 px-5 py-4 rounded-2xl border border-purple-500/25 bg-purple-500/8">
          ...
          <div className="flex-1">
            ...
            <p className="text-xs text-white/45 mt-0.5">Credited to <span className="text-white/60 font-mono">{user.username}</span></p>
          </div>
          <div className="text-2xl font-black text-purple-300">{user.gems.toLocaleString()} <span className="text-sm font-semibold">DC</span></div>
        </div>
      )}
