...
import {
  ...
  Coins, Gem, Settings, LogOut, Crown, Camera, RefreshCw,
  MapPin, Landmark, CheckCircle2, AlertCircle
} from "lucide-react";
...
export default function Profile() {
  ...
  if (!user) {
    return (
      <div className="flex flex-col items-center justify-center py-24 gap-6 text-center">
        ...
        <p className="text-white/40 max-w-xs text-sm">Sign in with your WhatsApp number to view your profile, balance, and purchase cards directly.</p>
  ...
  const pvpRatio = user.pvpLosses > 0 ? (user.pvpWins / (user.pvpWins + user.pvpLosses) * 100).toFixed(0) : "100";
  ...
  return (
    <div className="flex flex-col gap-6 pb-10">
        ...
        <div className="relative z-10 flex flex-col sm:flex-row items-start sm:items-center gap-5 p-6 sm:p-8">
          ...
          <div className="flex flex-row sm:flex-col gap-2.5 shrink-0">
            ...
            <div className="flex items-center gap-2 px-3 py-2 rounded-xl bg-amber-500/10 border border-amber-500/20">
              ...
              <div>
                <div className="text-[10px] text-white/40 leading-none">Solars</div>
                <div className="font-bold text-sm text-amber-300">{user.solars.toLocaleString()}</div>
              </div>
            </div>
            {/* Dark Crystals */}
            <div className="flex items-center gap-2 px-3 py-2 rounded-xl bg-purple-500/10 border border-purple-500/20">
              ...
              <div>
                <div className="text-[10px] text-white/40 leading-none">Dark Crystals</div>
                <div className="font-bold text-sm text-purple-300">{user.darkCrystals.toLocaleString()}</div>
              </div>
            </div>
            {/* Refresh button */}
            <button
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {[
          { label: "Solars (Wallet)", value: user.solars.toLocaleString(), icon: Coins, color: "text-amber-400", bg: "bg-amber-500/8 border-amber-500/15" },
          { label: "Dark Crystals", value: user.darkCrystals.toLocaleString(), icon: Gem, color: "text-purple-400", bg: "bg-purple-500/8 border-purple-500/15" },
          { label: "Mond", value: user.mond.toLocaleString(), icon: Star, color: "text-blue-400", bg: "bg-blue-500/8 border-blue-500/15" },
          { label: "Bank Solars", value: user.bankSolars.toLocaleString(), icon: Landmark, color: "text-green-400", bg: "bg-green-500/8 border-green-500/15" },
        ].map((item, i) => (
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {[
          { label: "Dungeons Cleared", value: user.dungeonsCleared.toLocaleString(), icon: Shield, color: "text-red-400" },
          { label: "PVP Wins", value: user.pvpWins.toLocaleString(), icon: Swords, color: "text-orange-400" },
          { label: "Cards Owned", value: user.cards.toLocaleString(), icon: Layers, color: "text-purple-400" },
      ...
      <div className="glass-panel overflow-hidden">
        ...
        <div className="divide-y divide-white/5">
          {[
            ...
            { label: "Guild",           value: user.guildId || "No Guild" },
